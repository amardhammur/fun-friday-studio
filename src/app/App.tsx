...
import { activityEvent, activitySegment, applyActivitySegment, applyEventUpdate, createEvent, createSegment, currentSegment, defaultEventTitle, eventDraft, segmentView } from '../core/event';
...
export function App({ initialSession, personalSession }: { initialSession: EventSession; personalSession?: EventSession }) {
  ...
  const exitDemo = (destination: 'home' | 'session' | 'people' = 'home') => {
    if (sessionRef.current.isDemo) install(structuredClone(personalRef.current));
    setRoute(destination);
  };
  const openPeople = () => exitDemo('people');
  const context: ActivityContext | undefined = activitySegmentView && { rosterLocked, segment: activitySegmentView, event: activityEventView, update, updateEvent: updateActivityEvent, notify, runTask, goHome: () => setRoute('home'), openPeople };
  ...
  const beginSetup = (chosen: Activity) => {
    ...
    if ((personal.segments.length > 0 || personal.scoreEntries.length > 0 || personal.phase !== 'lineup') && !window.confirm('Start a new one-activity event? Your current event will stay saved only if you export it first.')) return;
    ...
    updateEvent(s => {
      s.phase = 'segment'; prepareSegmentPeople(s, s.currentSegmentIndex);
    });
    setRoute('session');
  };
  const buildEvent = () => {
  ...
  const demo = (chosen: Activity) => void runTask('Preparing the demo…', async () => {
    const next = await createDemoEvent(chosen);
    install(next); setRoute('session');
  });
  const jumpStep = (id: string) => {
    if (!activity || !view) return false;
    if (rosterLocked && id !== 'game') { notify('People and teams are shared with completed activities. Import a replacement library to reset the event before changing the roster.'); return false; }
    const index = activity.setupSteps.findIndex(step => step.id === id);
    for (let i = 0; i < index; i++) { const issues = activity.setupSteps[i].validate(activitySegmentView!, activityEventView); if (issues.length) { notify(issues[0]); return false; } }
  ...
  return <div className={`app ${isStage ? 'presentation' : ''}`}><header className="app-header"><button className="brand" onClick={() => setRoute('home')} aria-label="Fun Friday Studio home"><span className="brand-mark"><BookOpen size={25}/><i>✦</i></span><span>fun friday<span className="brand-studio">STUDIO</span></span></button>{!isStage ? <nav className="main-nav" aria-label="Main navigation"><button className={route === 'home' ? 'active' : ''} onClick={() => setRoute('home')}><HomeIcon size={15}/> Activity library</button><button className={route === 'people' ? 'active' : ''} onClick={openPeople}><Users size={15}/> People library</button></nav> : <div className="stage-app-title"><span className="eyebrow">{activity?.name}</span></div>}<div className="header-actions"><span className={`offline-status ${offlineError ? 'warning-text' : ''}`} title={offlineReady ? 'All app, font, and model assets are cached on this laptop.' : import.meta.env.DEV ? 'Development server: use a production build to prepare the offline cache.' : offlineError ? 'Offline cache failed. Keep the local server running or retry online.' : 'Downloading the app assets for offline use…'}>{offlineReady ? <WifiOff size={14}/> : <ShieldCheck size={14}/>}<span>{offlineReady ? 'Offline ready' : import.meta.env.DEV ? 'Local & private' : offlineError ? 'Offline cache unavailable' : 'Preparing offline'}</span></span><button className="icon-button" title="Import session ZIP" aria-label="Import session ZIP" onClick={() => importInput.current?.click()}><Upload size={18}/></button><button className="button secondary header-export" onClick={() => void runTask('Packing your Friday memories…', () => exportSession(session))}><Download size={16}/><span>Export session</span></button>{isStage && <button className="icon-button" aria-label="Full screen" title="Full screen" onClick={() => void toggleFullscreen().catch(e => notify(e.message))}><Maximize size={19}/></button>}</div></header>
    ...
    {session.isDemo && <div className="demo-banner"><span>✦ DEMO</span> This demo is ready to play. Your people library is saved separately.
    ...
    {route === 'people' && <PeopleLibrary session={session} update={updateEvent} notify={notify} runTask={runTask} locked={libraryLocked(session)} continueLabel={afterPeopleStep && `Continue to ${afterPeopleStep.title.toLowerCase()}`} onContinue={!session.isDemo && session.phase === 'segment' && segment?.status === 'setup' && afterPeopleStep ? () => { if (jumpStep(afterPeopleStep.id)) setRoute('session'); } : undefined}/>}
