...
export function createSession(activity: Activity): AnySession {
  ...
  return { formatVersion: 1, id: crypto.randomUUID(), title: 'Our Fun Friday', activityId: activity.id, activityVersion: activity.version, segmentId: 'default', points: { correct: 2, steal: 1 }, createdAt: now, updatedAt: now, isDemo: false, phase: 'setup', setupStepId: activity.setupSteps[0].id, people: [], facePairs: [], teams: newTeams(), scoreEntries: [], settings: activity.defaultSettings(), game: activity.createInitialState(), assets: {} };
}
...
const schema = z.object({
  formatVersion: z.literal(1), id: z.string(), title: z.string(), activityId: z.string(), activityVersion: z.number().int().positive(), segmentId: z.string().default('default'), points: z.object({ correct: z.number().int(), steal: z.number().int() }).default({ correct: 2, steal: 1 }), createdAt: z.string(), updatedAt: z.string(), isDemo: z.boolean(), phase: z.enum(['setup', 'play', 'finale']), setupStepId: z.string(),
