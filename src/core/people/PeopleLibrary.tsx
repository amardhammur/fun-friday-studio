...
import { ArrowDown, ArrowUp, ArrowRight, Crop, Download, ImagePlus, Plus, Trash2, Users, UserPlus } from 'lucide-react';
...
type Props = { session: EventSession; update: (change: (draft: EventSession) => void) => void; notify: ActivityContext['notify']; runTask: ActivityContext['runTask']; locked: boolean; onContinue?: () => void; continueLabel?: string };
type Editing = { kind: 'group'; setId?: string } | { kind: 'crop'; setId: string } | { kind: 'person'; setId?: string };

export function PeopleLibrary({ session, update, notify, runTask, locked, onContinue, continueLabel }: Props) {
  ...
  return <main className="library-page"><div className="section-heading"><span className="eyebrow">THE FAMILIAR FACES</span><h1>Your people library<span className="accent">.</span></h1><p>One team roster, ready for every activity. Names and face pairs travel with your session.</p></div>
    <div className="names-toolbar"><span><Users size={18}/> {session.people.length} people · {session.people.filter(p => p.included).length} in the game</span><div className="button-row"><button className="button secondary" disabled={locked} onClick={() => setEditing({ kind: 'group' })}><Plus size={16}/> Add a group</button><button className="button secondary" disabled={locked} onClick={() => setEditing({ kind: 'person' })}><UserPlus size={16}/> Add a person</button><ImportPairsMenu session={session} update={update} notify={notify} runTask={runTask} locked={locked}/><button className="button secondary" disabled={!session.people.length} onClick={() => void runTask('Packing the face pairs…', () => exportFacePairs(session))}><Download size={16}/> Export pairs</button></div></div>
    {onContinue && <div className="setup-footer"><p className="muted">Changes are saved. Continue when your people are ready.</p><button className="button primary" onClick={onContinue}>{continueLabel ?? 'Continue to game setup'} <ArrowRight size={18}/></button></div>}
    ...
    {groups.map((set, index) => { const people = peopleInSet(session, set.id), needsPhotos = !set.nowImageId || !set.thenImageId; return <section className="library-set" key={set.id} aria-label={set.name}><header className="library-set-header"><div><span className="eyebrow">GROUP {index + 1}</span><input key={set.name} className="library-set-name" aria-label={`Name for ${set.name}`} maxLength={80} defaultValue={set.name} disabled={locked} onBlur={e => { const value = e.target.value; if (value.trim() && value.trim() !== set.name) update(s => { renameSet(s, set.id, value); }); else e.target.value = set.name; }}/><small className={needsPhotos ? 'warning-text' : undefined}>{needsPhotos ? 'Incomplete · Add photos' : `${people.length} people · ${people.filter(p => p.included).length} playing`}</small></div><div className="button-row"><button className="button subtle small-button" disabled={locked} onClick={() => update(s => setIncluded(s, set.id, true))}>Everyone in</button><button className="button subtle small-button" disabled={locked} onClick={() => update(s => setIncluded(s, set.id, false))}>Everyone out</button><button className="button secondary small-button" disabled={locked} onClick={() => setEditing({ kind: 'group', setId: set.id })}>{needsPhotos ? 'Resume setup' : 'Edit photos & matches'}</button><button className="icon-button" aria-label={`Move ${set.name} up`} disabled={locked || index === 0} onClick={() => update(s => moveSet(s, set.id, -1))}><ArrowUp size={16}/></button><button className="icon-button" aria-label={`Move ${set.name} down`} disabled={locked || index === groups.length - 1} onClick={() => update(s => moveSet(s, set.id, 1))}><ArrowDown size={16}/></button><button className="icon-button danger" aria-label={`Remove ${set.name}`} disabled={locked} onClick={() => remove(set.id, set.name)}><Trash2 size={16}/></button></div></header><div className="library-grid">{people.map(person => card(person))}</div>{!people.length && <p className="muted">{needsPhotos ? 'Your progress is saved. Resume setup to add photos, or remove this group.' : 'No one here yet. Use Edit photos & matches to finish this group.'}</p>}</section>; })}
    {singles.length > 0 && <section className="library-set" aria-label="Single photos"><header className="library-set-header"><div><span className="eyebrow">SINGLE PHOTOS</span><small>{singles.length} people · {singles.filter(set => peopleInSet(session, set.id)[0]?.included).length} playing</small></div></header><div className="library-grid">{singles.map(set => { const person = peopleInSet(session, set.id)[0]; return person && card(person, <div className="button-row"><button className="button subtle small-button" disabled={locked} onClick={() => setEditing({ kind: 'person', setId: set.id })}><ImagePlus size={15}/> Replace photos</button><button className="button subtle small-button" disabled={locked} onClick={() => setEditing({ kind: 'crop', setId: set.id })}><Crop size={15}/> Adjust crop</button><button className="icon-button danger" aria-label={`Remove ${set.name}`} disabled={locked} onClick={() => remove(set.id, set.name)}><Trash2 size={16}/></button></div>); })}</div></section>}
