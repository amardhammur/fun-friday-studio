...
export function Scoreboard({ session, update, activeTeamId, playedCounts = {} }: Pick<ActivityContext, 'session' | 'update'> & { activeTeamId?: string; playedCounts?: Record<string, number> }) {
  ...
  return <aside className="scoreboard"><div className="scoreboard-title"><Trophy size={19}/><h3>Office leaderboard</h3></div><p className="muted small">A little friendly competition.</p>
    {session.teams.map((team, i) => <div className={`score-row ${team.id === activeTeamId ? 'active' : ''}`} key={team.id}>
      ...
      <div className="score-meta"><span>{team.id === activeTeamId ? 'YOUR TURN' : scores[i] === lead && lead > 0 ? 'LEAD' : `${playedCounts[team.id] ?? 0} played`}{team.id === activeTeamId && scores[i] === lead && lead > 0 ? ' · LEAD' : ''}</span><div><button aria-label={`Subtract one point from ${team.name}`} onClick={() => update(s => { s.scoreEntries.push({ id: crypto.randomUUID(), teamId: team.id, kind: 'manual-adjustment', points: -1, active: true }); })}><Minus size={13}/></button><button aria-label={`Add one point to ${team.name}`} onClick={() => update(s => { s.scoreEntries.push({ id: crypto.randomUUID(), teamId: team.id, kind: 'manual-adjustment', points: 1, active: true }); })}><Plus size={13}/></button></div></div>
