...
export function MatchPeopleStep({ segment, event, update, updateEvent, runTask, notify }: Context) {
  ...
  const now = event.assets[nowId], then = event.assets[thenId];
  ...
  const pair = event.facePairs.find(p => p.id === selected), face = pair?.[side];
  const unmatched = event.facePairs.filter(p => !p.now || !p.then).length;
  ...
  const add = (imageSide: 'now' | 'then', rect: Rect) => {
    const id = crypto.randomUUID(), number = Math.max(0, ...event.facePairs.map(p => p.number)) + 1;
    updateEvent(s => { s.facePairs.push({ id, number, color: teamColors[(number - 1) % teamColors.length], [imageSide]: { sourceImageId: imageSide === 'now' ? nowId : thenId, faceBox: rect, padding: { ...defaultPadding } }, matchMethod: 'manual', reviewStatus: 'unmatched' }); syncPeople(s); });
  ...
  const detect = () => runTask('Finding the faces in your team…', async () => {
    try {
      ...
      const locked = event.facePairs.filter(p => p.matchMethod === 'manual' || p.reviewStatus === 'confirmed');
      ...
      const added = makePairs(newNow, newThen, nowId, thenId, segment.settings.matchingTolerance);
      ...
      updateEvent(s => { s.facePairs = [...locked, ...added]; syncPeople(s); });
  ...
  const editor = (imageSide: 'now' | 'then', isOverlay = false) => <PhotoEditor imageId={segment.game.previews[isOverlay || imageSide === 'now' ? nowId : thenId] || (imageSide === 'now' ? nowId : thenId)} overlayId={isOverlay ? segment.game.previews[thenId] || thenId : undefined} overlayOpacity={opacity} aspect={now.width / now.height} pairs={[...event.facePairs]} side={imageSide} mode={mode} selected={selected} onSelect={select} onChange={(id, rect) => change(id, imageSide, rect)} onAdd={rect => add(imageSide, rect)}/>;
  return <div className="setup-content wide"><div className="section-heading"><span className="eyebrow">02 / CONNECT THE DOTS</span><h1>Same smile, different year<span className="accent">.</span></h1><p>Find the faces. Check the pairs. Make everyone part of the story.</p></div>
    ...
    <div className="match-details"><div className="match-summary"><strong>{event.facePairs.filter(p => p.now && p.then).length} pairs</strong><span className={unmatched ? 'warning-text' : 'mint-text'}>{unmatched ? <><AlertTriangle size={16}/> {unmatched} unmatched — pair or remove these faces</> : <><Check size={16}/> All faces paired</>}</span></div><label className="tolerance">Match tolerance <input aria-label="Match tolerance" type="range" min=".02" max=".25" step=".01" value={segment.settings.matchingTolerance} onChange={e => update(s => { s.settings.matchingTolerance = +e.target.value; })}/><span>{Math.round(segment.settings.matchingTolerance * 100)}%</span></label></div>
    {face && pair && <div className="crop-controls"><strong>Face {pair.number} · {side}</strong>{(['x', 'y', 'width', 'height'] as const).map(key => <label key={key}>{key}<input aria-label={`Crop ${key}`} type="number" step=".1" min="0" max="100" value={+(face.faceBox[key] * 100).toFixed(1)} onChange={e => change(pair.id, side, { ...face.faceBox, [key]: +e.target.value / 100 })}/></label>)}<button className="button subtle" onClick={() => updateEvent(s => { const p = s.facePairs.find(p => p.id === pair.id)!; if (p.now && p.then) p.reviewStatus = 'confirmed'; })} disabled={!pair.now || !pair.then}><Check size={16}/> Confirm pair</button><button className="button danger subtle" onClick={() => { updateEvent(s => { const p = s.facePairs.find(p => p.id === pair.id)!; delete p[side]; p.reviewStatus = 'unmatched'; s.facePairs = s.facePairs.filter(f => f.now || f.then); syncPeople(s); }); setSelected(undefined); }}><Trash2 size={16}/> Delete this face</button></div>}
    <div className="setup-footer"><button className="button subtle" onClick={() => update(s => { s.setupStepId = 'upload'; })}>Back to photos</button><button className="button primary" disabled={!event.facePairs.some(p => p.now && p.then)} onClick={() => void runTask('Preparing your face pairs…', async () => { const next = await prepareCrops(segment, { ...event, people: [...event.people], facePairs: [...event.facePairs], teams: [...event.teams], scoreEntries: [...event.scoreEntries], assets: { ...event.assets } }); next.segment.setupStepId = 'names'; update(s => Object.assign(s, next.segment)); updateEvent(s => Object.assign(s, next.event)); })}>Name people <ArrowRight size={18}/></button></div>
  </div>;
}
