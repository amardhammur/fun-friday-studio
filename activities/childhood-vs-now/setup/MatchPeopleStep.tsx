...
export function MatchPeopleStep({ segment, event, update, updateEvent, runTask, notify }: Context) {
  ...
  if (!set || !now || !then) return <div className="empty-state"><h2>Two photos, one time machine.</h2><p>Upload both group photos first.</p><button className="button primary" onClick={() => update(s => { s.setupStepId = 'upload'; })}>Upload photos</button></div>;
  ...
  const add = (imageSide: 'now' | 'then', rect: Rect) => {
    ...
    updateEvent(s => { s.facePairs.push({ id, number, color: teamColors[(number - 1) % teamColors.length], setId: set.id, [imageSide]: { sourceImageId: imageSide === 'now' ? nowId : thenId, faceBox: rect, padding: { ...defaultPadding } }, matchMethod: 'manual', reviewStatus: 'unmatched' }); syncPeople(s); });
  ...
  const detect = () => runTask('Finding the faces in your team…', async () => {
    try {
      ...
      const added = makePairs(newNow, newThen, set, segment.settings.matchingTolerance);
  ...
  return <div className="setup-content wide"><div className="section-heading"><span className="eyebrow">02 / CONNECT THE DOTS</span><h1>Same smile, different year<span className="accent">.</span></h1><p>Find the faces. Check the pairs. Make everyone part of the story.</p></div>
    ...
    <div className="setup-footer"><button className="button subtle" onClick={() => update(s => { s.setupStepId = 'upload'; })}>Back to photos</button><button className="button primary" disabled={!event.facePairs.some(p => p.now && p.then)} onClick={() => void runTask('Preparing your face pairs…', async () => { const next = await prepareCrops(eventDraft(event)); update(s => { s.setupStepId = 'names'; }); updateEvent(s => Object.assign(s, next)); })}>Name people <ArrowRight size={18}/></button></div>
  </div>;
}
